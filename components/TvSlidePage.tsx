...
import { resolveActiveVideoUrl } from '../utils/tvSlideVideo.ts';
...
const TvSlidePage: React.FC<TvSlidePageProps> = ({ slide, fullscreen = false }) => {
  ...
  if (t === 'video') {
    /** Um único vídeo por visita ao slide — a rotação da playlist é feita no App ao paginar. */
    const mediaUrl = resolveActiveVideoUrl(slide);
    if (!mediaUrl) {
      return (
    ...
    const player = isLocalVideoRef(mediaUrl) ? (
      <LocalVideoTvPlayer key={mediaUrl} name={localVideoName(mediaUrl)} objectFit={videoFit} />
    ) : (
      <UploadedVideoTvPlayer key={mediaUrl} src={mediaUrl} objectFit={videoFit} />
    );
    if (fullscreen) {
