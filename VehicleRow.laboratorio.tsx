...
const VehicleRow: React.FC<VehicleRowProps> = ({ vehicle, isHighlighted, hasAnyHighlight, isAlerting }) => {
  ...
  const isFinalizado =
    s.includes('finalizado') || s.includes('pronto pra retirada') || s.includes('pronto para retirada');
  ...
  /** Produto encaminhado da oficina: aro âmbar (não sobrepõe o aro de garantia). */
  const showWorkshopRing = vehicle.fromWorkshop === true && !showGarantiaRing;
  ...
  const cardAccentStyle = useMemo((): React.CSSProperties => {
    ...
    if (showWorkshopRing) {
      shadows.push('0 0 0 5px rgb(245, 158, 11), 0 0 28px rgba(245, 158, 11, 0.45)');
    }
    if (shouldShake) {
    ...
    return style;
  }, [showGarantiaRing, showWorkshopRing, isHighlighted, shouldShake]);
  ...
  return (
    <div
      ...
      ${showGarantiaRing || showWorkshopRing ? 'overflow-visible' : 'overflow-x-hidden overflow-y-visible'}
    `}
      style={cardAccentStyle}
    >
      <div className="w-[22%] flex flex-col justify-center gap-1.5 overflow-visible font-black">
      ...
      <div className="w-[14%] border-l border-current/10 pl-4 pr-2 flex items-center justify-center h-full">
        {vehicle.benchSlot != null ? (
          <div className="flex items-center justify-center">
            <div className="flex h-[4.6rem] min-w-[4.6rem] flex-col items-center justify-center rounded-2xl border-2 border-current/30 bg-black/25 px-3 shadow-[0_4px_20px_rgba(0,0,0,0.35)] backdrop-blur-sm">
              <span className="text-[9px] font-black uppercase tracking-[0.3em] leading-none opacity-60">Comp.</span>
              <span className="font-black tabular-nums leading-none text-5xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.45)]">
                {vehicle.benchSlot}
              </span>
            </div>
          </div>
        ) : (
          <span className="text-3xl font-black opacity-25">—</span>
        )}
      </div>

      <div className="w-[14%] border-l border-current/10 pl-6 h-full flex items-center">
        <div className="flex flex-col uppercase font-black tracking-tight leading-[1.05] opacity-90">
          {(vehicle.productType || '—')
            .split(/\s+/)
            .filter(Boolean)
            .map((word, i) => (
              <span key={i} className="text-2xl truncate">{word}</span>
            ))}
        </div>
      </div>
